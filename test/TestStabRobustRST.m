...
TabL2RsR0=zeros(numel(nsubS),numel(degree));
TabH1sRsR0=zeros(numel(nsubS),numel(degree));
...
for ideg=1:numel(degree)
    ...
    for isubS=1:numel(nsubS)
        ...
        TabL2RsR0(isubS,ideg)=solutionRsR0.relErrL2;
        TabH1sRsR0(isubS,ideg)=solutionRsR0.relErrH1s;
        %
        methodData.regularityT = (methodData.degreeT-1);
        methodData.regularityS = (methodData.degreeS-1);
...
%% Save Table for Tikz
TabSumm=[Tabhtfhs,TabL2R0,TabH1sR0,TabL2R2,TabH1sR2,TabL2RsR0,TabH1sRsR0,TabL2R,TabH1sR];
...
for ideg=1:numel(degree)
    ...
    nameVars{ideg+1+5*numel(degree)}=sprintf('H1P%dRsR0',degree(ideg));
    nameVars{ideg+1+6*numel(degree)}=sprintf('L2P%dR',degree(ideg));
    nameVars{ideg+1+7*numel(degree)}=sprintf('H1P%dR',degree(ideg));
end
ErrorTable = array2table(TabSumm,'VariableNames',nameVars);
if saveTable
    writetable(ErrorTable,strcat('ErrorTableStabRobustRST.csv'));
end
