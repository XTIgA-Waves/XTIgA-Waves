...
TabL2RsR0=zeros(numel(nsub),numel(degree));
TabH1sRsR0=zeros(numel(nsub),numel(degree));
...
TabhpL2RsR0=Tabht.^(degree+1).*TabL2RsR0(end,:)./Tabht(end).^(degree+1);
TabhpH1sRsR0=Tabht.^(degree).*TabH1sRsR0(end,:)./Tabht(end).^(degree);

%% Save Table for Tikz
TabSumm=[Tabht,TabL2R0,TabH1sR0,TabL2R2,TabH1sR2,TabL2RsR0,TabH1sRsR0,TabhpL2R,TabhpH1sR,TabL2R,TabH1sR,TabhpL2R,TabhpH1sR];
nameVars=cell(1,12*numel(degree)+1);
nameVars{1}='ht';
for ideg=1:numel(degree)
    ...
    nameVars{ideg+1+4*numel(degree)}=sprintf('L2P%dRsR0',degree(ideg));
    nameVars{ideg+1+5*numel(degree)}=sprintf('H1sP%dRsR0',degree(ideg));
    nameVars{ideg+1+6*numel(degree)}=sprintf('h%dL2RsR0',degree(ideg)+1);
    nameVars{ideg+1+7*numel(degree)}=sprintf('h%dH1sRsR0',degree(ideg));
    nameVars{ideg+1+8*numel(degree)}=sprintf('L2P%dR',degree(ideg));
    nameVars{ideg+1+9*numel(degree)}=sprintf('H1sP%dR',degree(ideg));
    nameVars{ideg+1+10*numel(degree)}=sprintf('h%dL2R',degree(ideg)+1);
    nameVars{ideg+1+11*numel(degree)}=sprintf('h%dH1sR',degree(ideg));
end
ErrorTable = array2table(TabSumm,'VariableNames',nameVars);
if saveTable
    writetable(ErrorTable,strcat('ErrorTableStabConvRST.csv'));
end
