...
TabhpL2R0=Tabht.^(degree+1).*TabL2R0(end,:)./Tabht(end).^(degree+1);
TabhpH1sR0=Tabht.^(degree).*TabH1sR0(end,:)./Tabht(end).^(degree);

%% Save Table for Tikz
TabSumm=[Tabht,TabL2R0,TabH1sR0,TabL2R2,TabH1sR2,TabL2R,TabH1sR,TabhpL2R0,TabhpH1sR0];
...
for ideg=1:numel(degree)
    ...
    nameVars{ideg+1+6*numel(degree)}=sprintf('h%dL2R0',degree(ideg)+1);
    nameVars{ideg+1+7*numel(degree)}=sprintf('h%dH1sR0',degree(ideg));
end
ErrorTable = array2table(TabSumm,'VariableNames',nameVars);
if saveTable
    writetable(ErrorTable,strcat('ErrorTableStabZankPConvRST.csv'));
end
