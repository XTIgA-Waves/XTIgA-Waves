...
rhs(dofs) = rhs(dofs) + op_f_v(spSide,mshSide,dt_u_ex(x{:}));
...
switch Solver
    case 'dir'
        ...
        solution.u(trialIntDofs) = Mat\rhs(testForcedDofs);
    case 'dirEff'
        ...
        solution.u(trialIntDofs) = FTapplication(rhs(testForcedDofs),U,Ds,Q,Z,Tr,A);
        clear U Ds Q Z Tr A;
    case 'precT'
        [U,Ds,Q,Z,Tr,A] = FTparameters(Ks(intDofsS,intDofsS),Kt(1:end-1,2:end),Ms(intDofsS,intDofsS),Mt(1:end-1,2:end)-Pt(1:end-1,2:end));
        ...
        [y,~,relres,iter] = gmres(Mat,rhs(testForcedDofs),50,tol,numel(trialIntDofs),Prec);
        solution.u(trialIntDofs) = y;
        fprintf(1,'\n*Solver info: iter=[%d,%d]; Relres=%g*\n',iter(1),iter(2),relres);
        clear testForcedDofs U Ds Q Z Tr A Ks Kt Ms Mt Mr Wt intDofsS iter relres;
end
solution.nDof = numel(trialIntDofs);
clear trialIntDofs testForcedDofs rhs;
