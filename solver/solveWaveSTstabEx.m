...
rhs(dofs) = rhs(dofs) + op_fdotn_v (sp_side,msh_side,grad_u_ex_c2(x{:}));
clear msh_side x dofs;
...
for iside = rbnSides
    ...
    rhs(dofs) = rhs(dofs) + op_fdotn_v (sp_side,msh_side,grad_u_ex_c2(x{:}))+op_f_v (sp_side,msh_side,theta*c_dt_u_ex(x{:}));
end
