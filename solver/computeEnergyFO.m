function E = computeEnergyFO(qn,qw,u,v,c,spaceST,geometryST,map_der,tPts)

E = zeros(numel(tPts),1);
dimS = numel(qn)-1;
pts = [cellfun(@(qn)qn(:),qn(1:end-1),'UniformOutput',false),tPts];
...
v_eval = sp_eval(v,spaceST,geometryST,pts,{'value'});
v_eval = reshape(v_eval,[],2);
ndPts = cell(dimS,1);
...
for it = 1:2
    ...
    grad_x_l2_2 = sum(reshape(vecnorm(grad_u_eval(1:end-1,:,it),2,1),[],1).^2.*c_eval.^2.*wS.*abs(JacS))/2;
    grad_t_l2_2 = sum(reshape(v_eval(:,it),[],1).^2.*wS.*abs(JacS))/2;
